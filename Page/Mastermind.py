...
class Mastermind(Page):
    ...
    # TODO - finish converting these subs
    """
    sub mmBGActBGBind {
        ($profile,$filedn,$fileup,$action,$say,$powers) = @_

        MMP =    $profile.MastermindPets
        key =    MMP{"PetBackground$action"}
        method = MMP{"Pet${action}ResponseMethod"}

        ($bgact, $bgsay, tier1bg, tier2bg, tier3bg)
        #  fill bgsay with the right commands to have bodyguards say PetBodyguardResponse
        #  first check if any full tier groups are bodyguards.  full tier groups are eaither All BG or all NBG.
        ...
        if (((tier1bg + tier2bg + tier3bg) == 6) or ($method != 3)) {
            ...
            if (tier1bg == 3) {
                $bgsay = bgsay + '$$petsaypow ' + "$powers{'min'} $say"
            else :
                #  use petsayname commands for those tier1s that are bodyguards.
                $bgsay = bgsay + '$$petsayname ' + "MMP{'Pet1Name'} $say" if (MMP{'Pet1Bodyguard'})
                $bgsay = bgsay + '$$petsayname ' + "MMP{'Pet2Name'} $say" if (MMP{'Pet2Bodyguard'})
                $bgsay = bgsay + '$$petsayname ' + "MMP{'Pet3Name'} $say" if (MMP{'Pet3Bodyguard'})
            }
            if (tier2bg == 2) {
                $bgsay = bgsay + '$$petsaypow ' / "$powers{'lts'} $say"
            else :
                $bgsay = bgsay + '$$petsayname ' + "MMP{'Pet4Name'} $say" if (MMP{'Pet4Bodyguard'})
                $bgsay = bgsay + '$$petsayname ' + "MMP{'Pet5Name'} $say" if (MMP{'Pet5Bodyguard'})
            }
            if (tier3bg == 1) {
                $bgsay = bgsay + '$$petsaypow ' + "$powers{'bos'} $say"
            }
        }
        if ((tier1bg + tier2bg + tier3bg) == 6) {
            $bgact = '$$petcomall ' + $action
            ...
            if (tier1bg == 3) {
                $bgact = bgact + '$$petcompow ' + "$powers{'min'} $action"
            else :
                #  use petsayname commands for those tier1s that are bodyguards.
                $bgact = bgact + '$$petcomname ' + "MMP{'Pet1Name'} $action" if (MMP{'Pet1Bodyguard'})
                $bgact = bgact + '$$petcomname ' + "MMP{'Pet2Name'} $action" if (MMP{'Pet2Bodyguard'})
                $bgact = bgact + '$$petcomname ' + "MMP{'Pet3Name'} $action" if (MMP{'Pet3Bodyguard'})
            }
            if (tier2bg == 2) {
                $bgact = bgact + '$$petcompow ' + "$powers{'lts'} $action"
            else :
                $bgact = bgact + '$$petcomname ' + "MMP{'Pet4Name'} $action" if (MMP{'Pet4Bodyguard'})
                $bgact = bgact + '$$petcomname ' + "MMP{'Pet5Name'} $action" if (MMP{'Pet5Bodyguard'})
            }
            if (tier3bg == 1) {
                $bgact = bgact + '$$petcompow ' + "$powers{'bos'} $action"
            }
        }
    ...
    sub mmQuietBGSelBind {
        ($profile,$file,$powers) = @_
        MMP = $profile.MastermindPets
        if (MMP{'bg_enable'}) {
            ($bgset, tier1bg, tier2bg, tier3bg)
            #  fill bgsay with the right commands to have bodyguards say PetBodyguardResponse
            #  first check if any full tier groups are bodyguards.  full tier groups are eaither All BG or all NBG.
            if (MMP{'Pet1Bodyguard'}) { tier1bg = tier1bg + 1; }
            if (MMP{'Pet2Bodyguard'}) { tier1bg = tier1bg + 1; }
            ...
            if ((tier1bg + tier2bg + tier3bg) == 6) {
                ...
                if (tier1bg == 3) {
                    $bgset = bgset + '$$petcompow ' + "$powers{'min'} def fol"
                else :
                    #  use petsayname commands for those tier1s that are bodyguards.
                    $bgset = bgset + '$$petcomname ' + "MMP{'Pet1Name'} def fol" if (MMP{'Pet1Bodyguard'})
                    $bgset = bgset + '$$petcomname ' + "MMP{'Pet2Name'} def fol" if (MMP{'Pet2Bodyguard'})
                    $bgset = bgset + '$$petcomname ' + "MMP{'Pet3Name'} def fol" if (MMP{'Pet3Bodyguard'})
                }
                if (tier2bg == 2) {
                    $bgset = bgset + '$$petcompow ' + "$powers{'lts'} def fol"
                else :
                    $bgset = bgset + '$$petcomname ' + "MMP{'Pet4Name'} def fol" if (MMP{'Pet4Bodyguard'})
                    $bgset = bgset + '$$petcomname ' + "MMP{'Pet5Name'} def fol" if (MMP{'Pet5Bodyguard'})
                }
                if (tier3bg == 1) {
                    $bgset = bgset + '$$petcompow ' + "$powers{'bos'} def fol"
                }
            }
    ...
    sub mmQuietBGActBind {
        ($profile,$filedn,$fileup,$action,$powers) = @_

        MMP = $profile.MastermindPets
        key = MMP{"PetBackground$action"}

        ($bgact, tier1bg, tier2bg, tier3bg)
        #  fill bgsay with the right commands to have bodyguards say PetBodyguardResponse
        #  first check if any full tier groups are bodyguards.  full tier groups are eaither All BG or all NBG.
        if (MMP{'Pet1Bodyguard'}) { tier1bg = tier1bg + 1; }
        if (MMP{'Pet2Bodyguard'}) { tier1bg = tier1bg + 1; }
        ...
        if ((tier1bg + tier2bg + tier3bg) == 0) {
            ...
            if (tier1bg == 0) {
                ...
                if (not MMP{'Pet1Bodyguard'}) {
                    $bgact = bgact + '$$petcomname ' + "MMP{'Pet1Name'} $action"
                }
                if (not MMP{'Pet2Bodyguard'}) {
                    $bgact = bgact + '$$petcomname ' + "MMP{'Pet2Name'} $action"
                }
                if (not MMP{'Pet3Bodyguard'}) {
                    $bgact = bgact + '$$petcomname ' + "MMP{'Pet3Name'} $action"
                }
            }
            if (tier2bg == 0) {
                $bgact = bgact + '$$petcompow ' + "$powers{'lts'} $action"
                ...
                if (not MMP{'Pet4Bodyguard'}) {
                    $bgact = bgact + '$$petcomname ' + "MMP{'Pet4Name'} $action"
                }
                if (not MMP{'Pet5Bodyguard'}) {
                    $bgact = bgact + '$$petcomname ' + "MMP{'Pet5Name'} $action"
                }
            }
            if (tier3bg == 0) {
                $bgact = bgact + '$$petcompow ' + "$powers{'bos'} $action"
    ...
    #TODO - fix all these sigh
    """
    sub mmBGSubBind {
        ($profile,$filedn,$fileup,$fn,$powers) = @_
        MMP = $profile.MastermindPets
        %PetResponses
        for cmd (qw(SelectAll SelectMinions SelectLieutenants SelectBoss Bodyguard Aggressive Defensive Passive Attack Follow Stay Goto) ) {
        ...
        for cmd (qw(Aggressive Defensive Attack Follow Stay Goto)) {
    ...
    sub mmQuietSubBind {
        ...
        petcom = ($grp) ? "\$\$petcompow$grp" : '$$petcomall'
        for cmd (qw(Aggressive Defensive Attack Follow Stay Goto)) {
    ...
    sub mmQuietBGSubBind {
        ...
        for cmd (qw(Aggressive Defensive Passive Attack Follow Stay Goto)) {
