...
-- The form creation function takes an argument t, which is the table representing the bind command.
-- The form command must create a field, settings, on t, equal to the iup.frame containing all the UI allowing the CB user to change the settings for this bind command.
-- The form must then return t.

-- The new/creation function for a bind command creates the table used in the form and make functions.
-- any default settings should be placed in t as in t.method or t.power, etc.
-- this function also MUST supply a t.type variable, with the name of the Bind Command.
-- and it MUST NOT use t.settings, as this is used in the form function.  Otherwise, all other names are fair game, and even numbers can be used, like t[1], t[2], etc.

-- The make command simply creates the string that would be ordinarily typed inbetween $$'s in a bind command.
-- It can use any of the stuff set up in the new function, and anything set up in the UI functions from the form function.

-- Finally, addCmd has to be called to add the command functions to the list of bind commands that CB knows about.

-- A given PowerBind is essentially a table with integer index entries containing the commands to execute, in order, for the bind.  The powerbind table also has a container for IUp elements for dynamic changes

--creation functions for powerbind dialog goes here

function refreshCmds(bind,limit,profile,refreshcb)
	t = {}
	-- detach each command from the bind, then reattach.  this should be done after reordering or deleting.
