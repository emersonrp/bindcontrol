...
####### Window Save / Load
class ChatSaveLoadCmd(PowerBinderCommand):
    Name = "Chat Settings Save / Load"
    Menu = "Graphics / UI"
    ...
    def BuildUI(self, dialog) -> wx.BoxSizer:
        ...
        self.CommandChoice = wx.Choice(dialog, choices = [
            "Save Chat Settings to Specified File",
            "Load Chat Settings from Specified File",
            "Save Chat Settings to chat.txt",
            "Load Chat Settings from chat.txt",
        ])
        ...
        self.FilePath = wx.TextCtrl(dialog, size = wx.Size(400, -1),
            value = str(self.Profile.GameBindsDir() / 'chat.txt'))
    ...
    def MakeBindString(self) -> str:
        ...
        command = ['chatsavefile', 'chatloadfile', 'chatsave', 'chatload'][sel]
